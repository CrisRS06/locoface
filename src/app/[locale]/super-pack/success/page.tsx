...
import { Download, CheckCircle, Loader2, Clock, AlertCircle, Mail, Sparkles, Zap } from 'lucide-react';
...
export default function SuperPackSuccessPage() {
  ...
  const checkPackStatus = useCallback(async () => {
    ...
    try {
      // Reuse the same status endpoint (works for any credit_pack)
      ...
      if (data.status === 'paid' && data.hdUrl) {
        ...
        // Meta Pixel - Track Purchase event for Super Pack
        const purchaseKey = `fbq_purchase_super_pack_${packId}`;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center gradient-bg-soft">
        ...
        >
          ...
          <p className="text-slate-600">Loading your Super Pack...</p>
        </motion.div>
      </div>
    );
  }
  ...
  if (status === 'processing') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gradient-bg-soft p-4">
        ...
        >
          <div className="mx-auto w-16 h-16 bg-coral/20 rounded-full flex items-center justify-center mb-6">
            <Zap className="w-8 h-8 text-coral animate-pulse" />
          </div>

          <h1 className="text-2xl font-bold text-slate-900 mb-2">Activating Your Super Pack...</h1>
          ...
          <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
            <motion.div
              className="h-full bg-gradient-to-r from-coral to-orange-500"
              initial={{ width: '0%' }}
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gradient-bg-soft p-4 relative overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 -left-40 w-96 h-96 bg-coral opacity-40 rounded-full filter blur-3xl animate-blob" />
        <div className="absolute top-40 -right-40 w-96 h-96 bg-orange-400 opacity-30 rounded-full filter blur-3xl animate-blob animation-delay-2000" />
        <div className="absolute bottom-40 left-20 w-96 h-96 bg-yellow-300 opacity-20 rounded-full filter blur-3xl animate-blob animation-delay-4000" />
      </div>
      ...
      >
        ...
        <motion.div
          ...
          className="mx-auto w-20 h-20 bg-gradient-to-br from-coral/20 to-orange-400/30 rounded-full flex items-center justify-center mb-6 shadow-lg"
        >
        ...
        {hdUrl && (
          <motion.div
            ...
            className="mb-6 relative aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-coral/10 to-orange-400/10 border-2 border-white/50 shadow-inner"
          >
        ...
        <motion.div
          ...
          className="bg-coral/10 border border-coral/30 rounded-2xl p-5 mt-6"
        >
          ...
          <p className="text-sm text-slate-600">
            We sent your <strong>29 bonus codes</strong> to your email. Use them anytime to unlock more stickers!
          </p>
        </motion.div>
        ...
        >
          ...
          <ul className="space-y-2">
            ...
            <li className="flex items-center gap-2 text-sm text-slate-600">
              ...
              <span>29 promo codes sent to your email</span>
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Sparkles className="w-4 h-4 text-coral" />
